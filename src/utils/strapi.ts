export function prepareGroup(group: any | any[]): Group | Group[] {
  ...
  return {
    ...
    transactions: !transactions
      ? []
      : (prepareTransaction(transactions) as Transaction[]),
...
export function prepareTransaction(
  ...
  return {
    ...
    transactionMetas: !transaction_metas
      ? []
      : (prepareTransactionMeta(transaction_metas.data) as TransactionMeta[]),
  };
}

export function prepareTransactionMeta(
  transactionMeta: any | any[],
): TransactionMeta | TransactionMeta[] {
  if (Array.isArray(transactionMeta)) {
    return transactionMeta.map(
      (el) => prepareTransactionMeta(el) as TransactionMeta,
    );
  }
