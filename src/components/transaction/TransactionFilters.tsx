...
export default function TransactionFilters({
  ...
  onApply?: (event: React.KeyboardEvent | React.MouseEvent) => void;
}) {
  ...
  const [dateDisplay, setDateDisplay] = useState<(Dayjs | null)[]>([
    null,
    null,
  ]);

  const onAmountSliderChange = useCallback(
  ...
  const onAmountMinChange = useCallback(
  ...
  const onAmountMaxChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      let newValue =
        event.target.value === "" ? minAmountValue : Number(event.target.value);
      newValue = isNaN(newValue) ? maxAmountValue : newValue;
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <DatePicker
                  value={dateDisplay[0]}
                  onChange={(v: any) => {
                    setDate([v.format("YYYY-MM-DD"), date[1]]);
                    setDateDisplay([v, dateDisplay[1]]);
                  }}
                />
                <input type="hidden" name="date_from" value={date[0]} />
              </Grid>
              <Grid item xs={12} md={6}>
                <DatePicker
                  value={dateDisplay[1]}
                  onChange={(v: any) => {
                    setDate([date[0], v.format("YYYY-MM-DD")]);
                    setDateDisplay([dateDisplay[0], v]);
                  }}
                />
                <input type="hidden" name="date_to" value={date[1]} />
              </Grid>
            </Grid>
          </Box>
          <Box>
